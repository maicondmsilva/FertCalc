...
export default function Goals({ currentUser }: GoalsProps) {
  ...
  const calculateProgress = (goal: Goal) => {
    const relevant = pricings.filter((p) => {
      ...
      if (p.userId !== goal.userId || p.status !== 'Fechada' || p.approvalStatus !== 'Aprovada')
        return false;
  ...
  const handleAddGoal = async () => {
    if (!newGoal.targetValue || newGoal.targetValue <= 0) {
      showError('Insira um valor de meta válido.');
      return;
    }
    setLoading(true);
    try {
  ...
  const handleDeleteGoal = async (id: string) => {
    ...
    try {
      ...
      if (goalToDelete && currentUser.role === 'manager') {
        const masterAdmins = allUsers.filter(
          (u) =>
            u.role === 'master' ||
            u.role === 'admin' ||
            (u.permissions as any)?.approvals_canApprove === true
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 transition-all"
        >
        ...
        <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-sm space-y-4">
                  ...
                  {allUsers
                    .filter(
                      (u) =>
                        u.role === 'user' &&
                        (currentUser.role !== 'manager' ||
                          (currentUser.managedUserIds || []).includes(u.id))
                    )
                    .map((u) => (
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="px-4 py-2 text-stone-600 font-bold border border-stone-200 rounded-lg hover:bg-stone-50"
            >
              Cancelar
            </button>
            <button
              ...
              disabled={loading}
              className="px-4 py-2 bg-stone-800 text-white font-bold rounded-lg hover:bg-stone-900 disabled:bg-stone-400"
            >
              ...
              <div className="p-6 border-b border-stone-100">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-lg ${goal.type === 'monthly' ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600'}`}
                    >
                      {goal.type === 'monthly' ? (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-stone-500 uppercase font-bold tracking-wider">
                        {goal.type === 'monthly'
                          ? `${new Date(0, goal.month! - 1).toLocaleString('pt-BR', { month: 'long' })} / ${goal.year}`
                          : `Anual ${goal.year}`}
                      </p>
                    </div>
                  </div>
                  <span
                    className={`px-2 py-1 rounded-full text-[10px] font-black uppercase ${goal.status === 'Aprovada' ? 'bg-emerald-100 text-emerald-700' : goal.status === 'Reprovada' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}
                  >
                    {goal.status}
                  </span>
                </div>
                  ...
                  <div className="flex justify-between items-end mb-2">
                    <div>
                      <p className="text-3xl font-black text-stone-800">
                        ...
                        <span className="text-lg text-stone-500">t</span>
                      </p>
                  ...
                  {progress.remaining > 0 && (
                    <div className="bg-stone-50 p-3 rounded-xl border border-stone-100 flex items-center gap-3">
                      ...
                      <p className="text-xs text-stone-600">
                        Faltam <strong>{progress.remaining.toLocaleString('pt-BR')} Ton</strong>{' '}
                        para bater a meta.
                      </p>
                    </div>
                  )}
                ...
                <div className="flex gap-2">
                  {(currentUser.role === 'master' ||
                    ...
                    goal.status === 'Pendente' && (
                      <>
                        <button
                          onClick={() => handleApproveGoal(goal.id, 'Aprovada')}
                        ...
                        <button
                          onClick={() => handleApproveGoal(goal.id, 'Reprovada')}
