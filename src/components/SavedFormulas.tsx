...
import { Beaker, Trash2, ArrowRight, Save, Calendar, Database, User as UserIcon, Building2, Package, Zap, AlertTriangle } from 'lucide-react';
...
export default function SavedFormulas({ currentUser, onSendToCalculator }: SavedFormulasProps) {
    ...
    const handleDelete = async (id: string, name: string) => {
        if (!confirm(`Tem certeza que deseja excluir a fórmula "${name}"?`)) return;
    ...
    const getFormulaCost = (formula: SavedFormula, priceList: PriceList | undefined) => {
        ...
        formula.macros.forEach(macro => {
            ...
            const priceListItem = priceList.macros.find(m =>
                m.id === macro.id ||
                m.name.trim().toLowerCase() === macro.name.trim().toLowerCase()
            );
        ...
        formula.micros.forEach(micro => {
            ...
            const priceListItem = priceList.micros.find(m =>
                m.id === micro.id ||
                m.name.trim().toLowerCase() === micro.name.trim().toLowerCase()
            );
            if (priceListItem && priceListItem.price) {
                    ...
                    <div className="flex gap-2">
                        <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-stone-200 flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                                <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">Filial</span>
                                <select
                        ...
                        <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-stone-200 flex items-center gap-2">
                            ...
                            <div className="flex flex-col">
                                <span className="text-xs font-bold text-stone-400 uppercase tracking-wider">Tabela de Preços</span>
                                <select
                                    value={selectedPriceListId}
                                    onChange={(e) => setSelectedPriceListId(e.target.value)}
                                    className="bg-transparent text-stone-700 font-medium outline-none text-sm cursor-pointer"
                                >
                                    {availableLists.length > 0 ? availableLists.map(list => (
            ...
            {formulas.length === 0 ? (
                <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-12 text-center">
                    ...
                    <p className="text-stone-500 max-w-md mx-auto">
                        Você ainda não possui fórmulas salvas. Para criar uma predefinição, acesse a Calculadora, faça uma batida e clique em "Salvar Batida".
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {formulas.map(formula => {
                        ...
                        return (
                            <div key={formula.id} className="bg-white rounded-xl shadow-sm border border-stone-200 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                                <div className="p-5 border-b border-stone-100 flex-1">
                                    ...
                                    <div className="flex items-center text-xs text-stone-500 mb-4 gap-2">
                                        <span className="flex items-center gap-1"><UserIcon className="w-3 h-3" /> {formula.userName}</span>
                                        <span>•</span>
                                        <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {new Date(formula.date).toLocaleDateString('pt-BR')}</span>
                                    </div>

                                    <div className="bg-stone-50 p-3 rounded-lg border border-stone-100 mb-4">
                                        <div className="flex justify-between items-center mb-1">
                                            <p className="text-xs font-bold text-stone-500 uppercase tracking-widest">Fórmula Alvo</p>
                                            <p className="text-[10px] text-stone-400 font-mono">#{formula.id.slice(-4)}</p>
                                        </div>
                                        <p className="font-mono text-emerald-700 font-bold text-lg mb-3">{formula.targetFormula}</p>

                                        <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-1 border-t border-stone-200 pt-2">Composição (kg)</p>
                                        <div className="grid grid-cols-1 gap-1 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                                            {formula.macros.filter(m => m.quantity > 0).map(m => (
                                                <div key={m.id} className="flex justify-between items-center text-[11px]">
                                                    ...
                                                    <span className="font-mono font-bold text-stone-800">{m.quantity.toFixed(0)}</span>
                                                </div>
                                            ))}
                                            {formula.micros.filter(m => m.quantity > 0).map(m => (
                                                <div key={m.id} className="flex justify-between items-center text-[11px]">
                                                    ...
                                                    <span className="font-mono font-bold text-emerald-800">{m.quantity.toFixed(0)}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    {results.missingItems.length > 0 && selectedList && (
                                        <div className="mb-4 p-2 bg-red-50 border border-red-100 rounded-lg flex items-start gap-2">
                                            ...
                                            <div className="flex flex-col">
                                                <p className="text-[10px] font-bold text-red-600 uppercase tracking-tighter">Itens Não Disponíveis</p>
                                ...
                                <div className="bg-stone-50 p-3 border-t border-stone-100">
                                    <button
                                        onClick={() => onSendToCalculator(formula, selectedBranchId, selectedPriceListId)}
