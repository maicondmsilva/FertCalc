...
export const generatePricingPDF = (record: PricingRecord, settings: AppSettings, showAgent: boolean = true) => {
  ...
  el.innerHTML = `
    <!-- HEADER -->
    <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:3px solid #1a1a2e;padding-bottom:8px;margin-bottom:14px;">
      <div style="display:flex;align-items:center;gap:12px;">
        ${settings.companyLogo
      ? `<img src="${settings.companyLogo}" style="max-height:52px;max-width:120px;object-fit:contain;" />`
      : `<div style="width:52px;height:52px;background:#1a1a2e;border-radius:8px;display:flex;align-items:center;justify-content:center;"><span style="color:#fff;font-size:18px;font-weight:900;">${(settings.companyName || 'E')[0]}</span></div>`}
        <div>
          <div style="font-size:18px;font-weight:900;color:#1a1a2e;letter-spacing:-0.5px;">${settings.companyName || 'EMPRESA'}</div>
          ${settings.companyCnpj ? `<div style="font-size:10px;color:#555;">CNPJ: ${settings.companyCnpj}</div>` : ''}
        </div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:22px;font-weight:900;color:#1a1a2e;letter-spacing:1px;">PROPOSTA COMERCIAL</div>
    ...
    <!-- CLIENTE / AGENTE INFO -->
    <div style="display:grid;grid-template-columns:${showAgent ? '1fr 1fr' : '1fr'};gap:12px;margin-bottom:14px;">
      <div style="background:#f9fafb;border-radius:8px;padding:10px 14px;border-left:4px solid #1a1a2e;">
      ...
      ${showAgent ? `
      <div style="background:#f9fafb;border-radius:8px;padding:10px 14px;border-left:4px solid #4a90d9;">
    ...
    <div style="margin-bottom:16px;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        <thead>
          <tr style="background:#1a1a2e;color:#fff;">
            <th style="padding:8px 10px;text-align:left;font-size:10px;font-weight:700;border-radius:0;">Produto / Fórmula</th>
            <th style="padding:8px 10px;text-align:center;font-size:10px;font-weight:700;">Qtd. (ton)</th>
            <th style="padding:8px 10px;text-align:right;font-size:10px;font-weight:700;">Preço/ton</th>
            <th style="padding:8px 10px;text-align:right;font-size:10px;font-weight:700;">Frete/ton</th>
      ...
      ? calcs.map((calc, i) => `
              <tr style="background:${i % 2 === 0 ? '#fff' : '#f9fafb'};border-bottom:1px solid #eee;">
                <td style="padding:8px 10px;font-weight:600;font-size:11px;color:#1a1a2e;">${calc.formula || record.factors?.targetFormula || '—'}</td>
                <td style="padding:8px 10px;text-align:center;font-size:11px;font-weight:700;">${fmtN(calc.factors?.totalTons || 0)}</td>
                <td style="padding:8px 10px;text-align:right;font-size:11px;">${fmt(calc.summary?.finalPrice || 0)}</td>
                <td style="padding:8px 10px;text-align:right;font-size:11px;">${fmt(freight)}</td>
                <td style="padding:8px 10px;text-align:right;font-weight:700;font-size:11px;">${fmt(calc.summary?.totalSaleValue || 0)}</td>
              </tr>`).join('')
      : `<tr><td colspan="5" style="padding:16px;text-align:center;color:#aaa;font-style:italic;">Nenhuma fórmula calculada</td></tr>`}
          <tr style="background:#1a1a2e;color:#fff;">
            <td style="padding:8px 10px;font-weight:800;font-size:11px;" colspan="2">TOTAL GERAL</td>
            <td style="padding:8px 10px;text-align:center;font-weight:800;font-size:11px;">${fmtN(totalTons)} ton</td>
            <td style="padding:8px 10px;"></td>
            <td style="padding:8px 10px;text-align:right;font-weight:900;font-size:13px;">${fmt(grandTotal)}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
      <div style="background:#f9fafb;border-radius:8px;padding:10px 14px;">
        <div style="font-size:9px;font-weight:800;text-transform:uppercase;color:#888;margin-bottom:6px;letter-spacing:0.5px;">Condições de Pagamento</div>
        <div style="display:flex;flex-direction:column;gap:4px;">
          <div style="font-size:10px;"><span style="color:#888;">Vencimento:</span> <strong>${dueDate}</strong></div>
          <div style="font-size:10px;"><span style="color:#888;">Tipo de Frete:</span> <strong>${freightLabel}</strong></div>
        </div>
      </div>
    ...
    <!-- TERMOS COMERCIAIS -->
    <div style="background:#f0f4f8;border-radius:8px;padding:10px 14px;border-top:3px solid #1a1a2e;">
    ...
    <!-- FOOTER -->
    <div style="margin-top:30px;padding-top:8px;border-top:1px solid #eee;display:flex;justify-content:space-between;align-items:center;">
