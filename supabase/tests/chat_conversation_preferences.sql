...
begin
  ...
  perform set_config('request.jwt.claim.sub', '49000000-0000-4000-8000-000000000002', true);
  perform public.get_or_create_direct_chat('49000000-0000-4000-8000-000000000001');
  perform set_config('request.jwt.claim.sub', '49000000-0000-4000-8000-000000000001', true);

  if not exists (
    select 1 from public.list_chat_conversations(50, true) conversation
    where conversation.conversation_id = target_conversation_id
      and conversation.archived_at is not null
  ) then
